...
PHENO_ID="$(dx upload "${PHENO_LOCAL}" --path "${DEST_INPUTS}" --brief --parents --overwrite)"
...
# Upload manifest to DNAnexus persist folder
dx upload "${MANIFEST_LOCAL}" --path "${DEST_PERSIST}" --parents --overwrite >/dev/null
echo "[OK] Persisted manifest to: ${DEST_PERSIST}/manifest_ids.env"

# Write / refresh state env locally (job IDs get appended in later stages)
